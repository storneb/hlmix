...
public Plugin:myinfo =
{
	name = "AUTOMIX-Plugin",
	author = "automaric",
	description = "Facilita el trabajo del administrador al ejecutar una combinación (partida de equipo o guerra de clanes) y el sistema Auto-Mix.",
...
public Action:InformPlayerAboutTheMix(Handle:timer, any:client)
{
	if (hasMixStarted)
	{
		PrintToChat(client, "\x04[%s]:x\03 El partido comienza en \x043!", MODNAME);
		PrintToChat(client, "\x04[%s]:x\03 El partido comienza en \x042!", MODNAME);
		PrintToChat(client, "\x04[%s]:x\03 El partido comienza en \x041!", MODNAME);
		PrintToChat(client, "\x04[%s]:x\03 \x01El partido ya comenzo! Buena suerte y que te diviertas", MODNAME);
	}
}
...
public Action:Command_GagPlayer(client, args) {
	...
	if (count <= 0)
	{
		PrintToChat(client,"\x04[%s] |\x03No hay tal objetivo %d", MODNAME, pattern);
		return Plugin_Handled;
	}
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			PrintToChat(client, "\x04[%s]:\x03 Te has amordazado %s !", MODNAME, targetName);
			PrintToChat(targets[i], "\x04[%s]:\x03 Has sido amordazado!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_UnGagPlayer(client, args) {
	...
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			GetClientName(targets[i], targetName, sizeof(targetName));
			PrintToChat(client, "\x04[%s]:\x03 Te has desahogado %s !", MODNAME, targetName);
			PrintToChat(targets[i], "\x04[%s]:\x03 Has sido des-amordazado!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_MutePlayer(client, args)
{
	...
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			GetClientName(targets[i], targetName, sizeof(targetName));
			PrintToChat(client, "\x04[%s]:\x03 Has silenciado a %s !", MODNAME, targetName);
			PrintToChat(targets[i], "\x04[%s]:\x03 Has sido silenciado!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_UnMutePlayer(client, args)
{
	...
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			GetClientName(targets[i], targetName, sizeof(targetName));
			PrintToChat(client, "\x04[%s]:\x03 Has desmuteado a %s !", MODNAME, targetName);
			PrintToChat(targets[i], "\x04[%s]:\x03 Lo han desmuteado!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_Last(client, args)
{
	...
	{
		PrintToChat(client, "\x04[%s]:\x03 Ningun jugador se ha unido desde que se cargo el mix...", MODNAME);
		return Plugin_Handled;
	}
	
	PrintToChatAll("\x04[%s]:\x03 El ultimo jugador que se unio es:", MODNAME);
...
public OnMapStart()
{
	...
	if (g_iAccount == -1)
		PrintToChatAll("\x04[%s]:\x03 No puedo encontrar la m_iAccount ! - El dinero no se mostrara en el mix!", MODNAME);
		
	g_hRestartGame = FindConVar("mp_restartgame");
	if (g_hRestartGame == INVALID_HANDLE)
	{
		PrintToChatAll("\x04[%s]:\x03 No se pudo encontrar el cvar mp_restartgame !", MODNAME);
		SetFailState("[%s]: No se pudo encontrar el cvar mp_restartgame !", MODNAME);
	}
	HookConVarChange(g_hRestartGame, OnGameRestarted);
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	*/
		...
		if (IsClientInGame(i))
		{
			...
			PrintToChat(i, "\x04[%s]:\x03 Se actualizaron sus armas, dinero y puntuacion!", MODNAME);
		}
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (isKo3Running) // Knife is running and its own text will be shown.
		{
			if (GetConVarInt(g_CvarHalfAutoLiveStart) == 1)
				PrintToChatAll("\x04[%s]:\x03 Ko3 se esta ejecutando...", MODNAME);
			else if (hasMixStarted)
				PrintToChatAll("\x04[%s]:\x03 Ko3 se esta ejecutando... escribe !live para comenzar el match", MODNAME);
			...
			PrintToChatAll("\x04[%s]:\x03 Esta ronda es decisiva para saber quien elegira el bando.", MODNAME);
			return;
		}
		...
		if (hasMixStarted)
		{
			...
			if (g_CurrentHalf == 1)
			{
				...
				if (GetConVarInt(g_CvarShowScores) == 1)
					PrintToChatAll("\x04[%s]:\x03 Ronda\x03 %d \x04- Mitad\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScoreH1, teamBName, g_nTScoreH1);
			...
			else if (g_CurrentHalf == 2)
			{
				...
				else
				{
					...
					if (GetConVarInt(g_CvarShowScores) == 1)
						PrintToChatAll("\x04[%s]:\x03 Ronda\x03 %d \x04- Mitad\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);
					...
					if ((g_nCTScore == 15) && (g_nTScore == 15) && (GetConVarInt(g_CvarMr3Enabled) == 1)) // If roundEnd failed at starting the mr3 settings...
					{
						...
						if (GetConVarInt(g_CvarHalfAutoLiveStart) == 0)
							PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. Se cargaran los ajustes de Mr3! \x01- \x03Escribe\x04 !live \x03para comenzar.", MODNAME);
						else
							PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. Se cargaran los ajustes de Mr3!", MODNAME);
					}
					else
					{
						if (GetConVarInt(g_CvarMr3Enabled) == 1)
						{
							if ((g_nCTScore == 15) && (g_nTScore == 14))
								PrintToChatAll("\x04[%s]:\x03 LR \x04porque\x03 %s - \x04si %s ganan, Mr3 sera cargado!", MODNAME, teamAName, teamBName);
							else if ((g_nCTScore == 14) && (g_nTScore == 15))
								PrintToChatAll("\x04[%s]:\x03 LR \x04porque\x03 %s - \x04si %s ganan, Mr3 sera cargado!", MODNAME, teamBName, teamAName);
							else
							{
								if (g_nCTScore == 15)
									PrintToChatAll("\x04[%s]:\x03 LR \x04para\x03 %s", MODNAME, teamAName);
								if (g_nTScore == 15)
									PrintToChatAll("\x04[%s]:\x03 LR \x04para\x03 %s", MODNAME, teamBName);
							}
						}
						else
						{
			...
			else if (g_CurrentHalf > 2)
			{
				...
				if (GetConVarInt(g_CvarShowScores) == 1)
					PrintToChatAll("\x04[%s]:\x03 Ronda\x03 %d \x04- Mitad\x03 %d\x04 /\x03 4\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);
			...
			{	
				...
				if ((g_CurrentHalf == 1) && (g_CurrentRound == 16))
				{
					...
					if (GetConVarInt(g_CvarShowSwitchInPanel) == 1) // 0 = in chat, 1 = in panel.
					{
						...
						DrawPanelText(panel, " Los equipos se intercambiaran AUTOMATICAMENTE despues de esta ronda. \n *NO* \n cambies tu equipo.");
						DrawPanelItem(panel, "", ITEMDRAW_SPACER);

						SetPanelCurrentKey(panel, 10);
						DrawPanelItem(panel, "Cerrar", ITEMDRAW_CONTROL);
						...
						PrintToChatAll("\x04[%s]:\x03 Los equipos se intercambiaran AUTOMATICAMENTE despues de esta ronda.. \x01NO\x03 cambies tu equipo.", MODNAME);
				}
...
ShowTeamMoneyAndWeapons()
{
	if ((hasMixStarted) && (g_iAccount != -1))
	{
		...
		else if(show == 1) // Show team money in panel ...
		{
			...
			DrawPanelItem(panelT, "-------------------------", ITEMDRAW_SPACER);
			DrawPanelItem(panelT, "Cerrar", ITEMDRAW_CONTROL);
			SetPanelCurrentKey(panelT, 10);
			DrawPanelItem(panelCT, "-------------------------", ITEMDRAW_SPACER);
			DrawPanelItem(panelCT, "Cerrar", ITEMDRAW_CONTROL);
...
public Handle_TeamsVoteMenu(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		isKo3Running = false;
		PrintToChatAll("x04[%s]:\x03 Los equipos se han decidido!");
		Command_Mr15(0, 0);
	}
}
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (isKo3Running) // Knife round has been ended...
		{
			if ((GetConVarInt(g_CvarKnifeWinTeamVote) == 1)) // Lets vote for the side of the teams.
			{
				...
				if (win_team == TEAM_T)
					SetMenuTitle(switchteams, "Cambiar de lado al equipo CT?");
				else if (win_team == TEAM_CT)
					SetMenuTitle(switchteams, "Cambiar de lado al equipo T?");
				AddMenuItem(switchteams, "Si", "Si");
				...
				if (win_team == 2)
					PrintToChatAll("\x04[%s]:\x03 Los terroristas elegiran ahora su equipo!", MODNAME);
				else if (win_team == 3)
					PrintToChatAll("\x04[%s]:\x03 Los Anti-Terroristas elegiran ahora su equipo!", MODNAME);
		...
		if ((hasMixStarted) && (didLiveStarted)) // Add 1 to the current round win number
		{
			...
			if (GetConVarInt(g_CvarShowMVP) == 1)
			{
				...
				if (kills == 5)
					PrintToChatAll("\x04[%s]:\x03 Ronda\x01 %d \x03MVP:\x04 %s , \x03Hizo un \x04ACE!", MODNAME, g_CurrentRound-1, attackerName, kills);
				else if (kills == 4)
					PrintToChatAll("\x04[%s]:\x03 Ronda\x01 %d \x03MVP:\x04 %s , \x03Hizo un \x04MINI!", MODNAME, g_CurrentRound-1, attackerName, kills);
				else if (kills == 3)
					PrintToChatAll("\x04[%s]:\x03 Ronda\x01 %d \x03MVP:\x04 %s , \x03Enemigos:\x04 %d \x03Muertos!", MODNAME, g_CurrentRound-1, attackerName, kills);
			}
		}
		...
		if ((hasMixStarted) && (g_SwapNow) && (didLiveStarted) && (g_CurrentHalf == 1))
		{
			...
			PrintToChatAll("\x04[%s]:\x03 Intercambiando equipos...", MODNAME);
			...
			if (GetConVarInt(g_CvarHalfAutoLiveStart) == 0)
				PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. MITAD 2! \x01- \x03Escribe\x04 !live \x03para comenzar.", MODNAME);
			else
			{
				new executed = 0;
				...
				PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. MITAD 2 Comenzara en unos segundos!", MODNAME);
				PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. MITAD 2 Comenzara en unos segundos!", MODNAME);
				PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. MITAD 2 Comenzara en unos segundos!", MODNAME);
			}
		...
		else if ((hasMixStarted) && (didLiveStarted))
		{
			...
			if ((g_CurrentHalf == 3) && (g_SwapNow) && (g_nCTScore < 19) && (g_nTScore < 19))
			{
				...
				if (GetConVarInt(g_CvarHalfAutoLiveStart) == 0)
					PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. ULTIMA RONDA! \x01- \x03Escribe\x04 !live \x03para comenzar.", MODNAME);
				else
					PrintToChatAll("\x04[%s]:\x03 Equipos intercambiados. ULTIMA RONDA!", MODNAME);
				g_SwapNow = false;
			}
...
public Action:Command_TvRecord(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			PrintToChat(client, "\x04[%s]:\x03 Source-Tv ya esta grabando. Puede detener el registro o esperar hasta el final del match", MODNAME);
			return Plugin_Handled;
		}
...
public Action:Command_Pcw(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (hasMixStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 El match ya se esta ejecutando.", MODNAME);
			return Plugin_Handled;
		}
		
		if (GetConVarInt(g_CvarShowMVP) != 1)
		{
			PrintToChat(client, "\x04[%s]:\x03 Para usar sm_pcw, se deben mostrar las puntuaciones MVP!", MODNAME);
			return Plugin_Handled;
		}
		...
		PrintToChat(client, "\x04[%s]:\x03 Este complemento esta deshabilitado para habilitarlo escribe: \x01sm_mixmod_enable 1", MODNAME);
...
public Action:Command_Stop(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			isKo3Running = false;
			PrintToChat(client, "\x04[%s]:\x03 Se ha deshabilitado la ronda de cuchillos!", MODNAME);
			return Plugin_Handled;
		}

		if (!hasMixStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 El match no se esta ejecutando...", MODNAME);
			return Plugin_Handled;
		}
		...
		if (GetConVarInt(g_CvarStopCustomCfg) == 1)
		{
			if (FileExists("cfg/stop.cfg"))
			{
				...
				PrintToChatAll("\x04[%s]:\x03 No se puedo encontrar: stop.cfg por lo que se ejecutara prac.cfg o warmup.cfg en su lugar!", MODNAME);
		}
...
public Action:Command_Live(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (didLiveStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 El match esta ejecutando LIVE!", MODNAME);
			return Plugin_Handled;
		}
		if (!hasMixStarted)
		{
		...
		PrintToChatAll("\x04[%s]:\x03 El match ya esta en LIVE!", MODNAME);
...
public Action:Command_NotLive(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (!didLiveStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 El match no esta LIVE...", MODNAME);
			return Plugin_Handled;
		}
		if (!hasMixStarted)
		{
...
public Action:Command_RandomTeams(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (hasMixStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 El match ya se esta ejecutando. No puedes hacer equipos aleatorios ahora!", MODNAME);
			return Plugin_Handled;
		}
		...
		PrintToChatAll("\x04[%s]:\x03 Los equipos han sido seleccionado al azar!", MODNAME);
...
RandomCTPlayers()
{
	...
	while(numSwitched < 5)
	{
		...
		else
		{
			...
			PrintToChatAll("\x04[%s]:\x01 ERROR: No se pudo obtener un jugador aleatorio del equipo T!", MODNAME);
			break;
		}
	}
}
...
public Action:Command_Pause(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		g_CurrentRound--;
		PrintToChatAll("\x04[%s]:\x03 Esta ronda no se contara!", MODNAME);
...
public Action:InformMix(Handle:timer, any:team)
{
	...
	PrintToChatAll("\x04--- Mix-Plugin creado por automaric ---");
	PrintToChatAll("\x04[%s]:\x03 El match ha terminado!", MODNAME);
	if (team == 3)
		PrintToChatAll("\x04[%s]: \x03El ganador es:\x04 %s", MODNAME, teamAName);
	else if (team == 2)
		PrintToChatAll("\x04[%s]: \x03El ganador es:\x04 %s", MODNAME, teamBName);
	else if (team == 1)
		PrintToChatAll("\x04[%s]: \x03No hubo ganador.\x04 ES UN EMPATE!", MODNAME);
...
public Action:DisplayScores(Handle:timer)
{
	...
	if (win < 1)
	{
		PrintToChatAll("\x04[%s]:\x03 Hubo un error al intentar encontrar el reproductor MVP...", MODNAME);
		return;
	}
	decl String:winnerName[33];
	GetClientName(win, winnerName, sizeof(winnerName));
	new kills = g_ScoresOfTheGame[win];
	PrintToChatAll("\x04[%s]:\x03 Estadisticas de muertes:", MODNAME);
	PrintToChatAll("------------------");
	PrintToChatAll("\x03 - El \x01MVP:\x04 %s , \x03Mato:\x04 %d \x03Enemigos!", winnerName, kills);
...
CreateWinningTeamPanel(team)
{
	...
	DrawPanelItem(g_WinTeamPanel, "Cerrar", ITEMDRAW_CONTROL);
...
public Action:Command_Mr15(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (FileExists(cfgPath))
			...
			PrintToChatAll("\x04[%s]:\x03 No se pudo ejecutar el %s / match / mr15 / live / esl5on5 config!", MODNAME, cfgName);
...
public Action:Command_Prac(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (FileExists(cfgPath))
			...
			PrintToChatAll("\x04[%s]:\x03 No se pudo ejecutar el %s / warmup / prac config!", MODNAME, cfgName);
...
public Action:Command_Mr3(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (GetConVarInt(g_CvarMr3Enabled) == 1)
		{
			...
			{
				ServerCommand("exec %s", cfgName);
				PrintToChatAll("\x04[%s]:\x03 %s Se ha cargado la configuracion...", MODNAME, cfgName);
			}
			else if (FileExists("cfg/mr3.cfg"))
			{
				ServerCommand("exec mr3");
				PrintToChatAll("\x04[%s]:\x03 MR3 Config ha sido cargado...", MODNAME);
			}
			else
				PrintToChatAll("\x04[%s]:\x03 No se pudo ejecutar la configuracion MR3!", MODNAME);
		}
		else PrintToChat(client, "\x04[%s]:\x03 MR3 esta deshabilitado... la configuración de mr3 no se cargara!", MODNAME);
...
public Action:Command_KO3(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			PrintToChatAll("\x04[%s]:\x03 El match esta LIVE... La configuracion ko3 no se cargara!", MODNAME);
			return Plugin_Handled;
		}
		...
		PrintToChatAll("\x04[%s]:\x03 RONDA A FAKA!", MODNAME);
		PrintToChatAll("\x04[%s]:\x03 RONDA A FAKA!", MODNAME);
		PrintToChatAll("\x04[%s]:\x03 RONDA A FAKA!", MODNAME);
		PrintToChatAll("\x04[%s]:\x03 RONDA A FAKA!", MODNAME);
		PrintToChatAll("\x04[%s]:\x03 RONDA A FAKA!", MODNAME);
	...
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarEnablePasswords) == 1))
	{
		if (args < 1)
		{
			...
			if (GetUserAdmin(client) != INVALID_ADMIN_ID)
				PrintToChatAll("\x04[%s]:\x03 La contraseña del servidor es: %s", MODNAME, sPass);
			else
			{
				if ((isRandomPasswordWasLastPw) && (GetConVarInt(g_CvarRpwShowPass) == 0))
				{
					PrintToChat(client, "\x04[%s]:\x03 La contraseña del servidor es: ***RANDOM***", MODNAME);
					if (GetUserAdmin(client) != INVALID_ADMIN_ID)
						PrintToChat(client, "\x04*** %s", sPass);
				}
				else
					PrintToChat(client, "\x04[%s]:\x03 La contraseña del servidor es: %s", MODNAME, sPass);
			}
		}
		else
		{
			if (GetUserAdmin(client) != INVALID_ADMIN_ID)
			{
				...
				PrintToChatAll("\x04[%s]:\x03 La contraseña del servidor es ahora: %s", MODNAME, pass);
				isRandomPasswordWasLastPw = false;
			}
			else
				PrintToChat(client, "[SM] Solos los administradores pueden cambiar la contraseña!");
		}
...
public Action:Command_GenerateRandomPassword(client, args)
{
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarEnablePasswords) == 1))
	{
		...
		if ((GetConVarInt(g_CvarRpwShowPass) == 1) || (!g_IsItManual && (client == 0))) // Show the password
			PrintToChatAll("\x04[%s]:\x03 La contraseña del servidor es: %d", MODNAME, rnd);
		else if (GetConVarInt(g_CvarRpwShowPass) == 0) // Don't show the password
		{
			PrintToChatAll("\x04[%s]:\x03 La contraseña del servidor es: ***RANDOM***", MODNAME);
			// Show the real password to the admin.
			if (client != 0)
				PrintToChat(client, "\x04*** %d", rnd);
			else
				PrintToServer("[%s]: Contraseña aleatoria: %d", rnd);
...
public Action:Command_RemovePass(client, args)
{
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarEnablePasswords) == 1))
	{
		...
		PrintToChatAll("\x04[%s]:\x03 La contraseña ha sido borrada!", MODNAME);
		isRandomPasswordWasLastPw = false;
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (args == 0)
		{
			...
			for(new i=1;i<=MaxClients;i++)
			{
				...
				{
					ChangeClientTeam(i, 3);
					PrintToChat(client, "\x04[%s]:\x03 Has sido intercambiado a \x01 Counter Terrorists\x03 Team.", MODNAME);
				}
				else if (team == 3)
				{
					ChangeClientTeam(i, 2);
					PrintToChat(client, "\x04[%s]:\x03 Has sido intercambiado a \x01 Terrorists\x03 Team.", MODNAME);
				}
			}
			PrintToChatAll("\x04[%s]:\x03 Todos los jugadores han sido intercambiados.", MODNAME);
			...
			if (found > 0) // Target is found!
			{
				...
				for (new target = 0; target < found; target++)
				{
					if (GetClientTeam(targetsArr[target]) != team)
					{
						...
						PrintToChat(targetsArr[target], "\x04[%s]:\x03 Tu equipo ha sido cambiado!", MODNAME);
					}
				}
				if (flag)
					didLiveStarted = true;
					
				PrintToChat(client,"\x04[%s]:\x03 %s el equipo ha sido cambiado!", MODNAME, searchFor);
			}
			else 
				PrintToChat(client,"\x04[%s]:\x03No se pudo encontrar %s en el servidor...", MODNAME, searchFor);
...
public Action:ShowScores(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (!IsFakeClient(client))
		{
			if (!hasMixStarted)
			{
				PrintToChat(client, "\x04[%s]:\x03 El match aun no se esta ejecutando...", MODNAME);
				return Plugin_Handled;
			}
			...
			if (g_CurrentHalf == 1)
			{
				PrintToChat(client, "\x04[%s]:\x03 Ronda\x03 %d \x04- Mitad\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScoreH1, teamBName, g_nTScoreH1);
			...
			{
				PrintToChat(client, "\x04[%s]:\x03 Ronda\x03 %d \x04- Mitad\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);

				if (GetConVarInt(g_CvarMr3Enabled) == 1)
				{
					if ((g_nCTScore == 15) && (g_nTScore == 14))
						PrintToChatAll("\x04[%s]:\x03 LR \x04por\x03 %s - \x04si %s gana, Mr3 se cargara!", MODNAME, teamAName, teamBName);
					else if ((g_nCTScore == 14) && (g_nTScore == 15))
						PrintToChatAll("\x04[%s]:\x03 LR \x04por\x03 %s - \x04si %s gana, Mr3 se cargara!", MODNAME, teamBName, teamAName);
					else
					{
						if (g_nCTScore == 15)
							PrintToChatAll("\x04[%s]:\x03 LR \x04para\x03 %s", MODNAME, teamAName);
			...
			{
				PrintToChat(client, "\x04[%s]:\x03 Ronda\x03 %d \x04- Mitad\x03 %d\x04 /\x03 4\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);
...
public Action:Command_SetMixScore(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1 && (ENABLE_PLUGIN_CHECKING))
	{
		...
		PrintToChat(client, "Se han establecido las puntuaciones!");
...
public Action:Command_KickCT(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		PrintToChatAll("\x04[%s]:\x03 El equipo Anti-Terrorista ha sido expulsado!", MODNAME);
...
public Action:Command_KickT(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		PrintToChatAll("\x04[%s]:\x03 El equipo Terrorista ha sido expulsado!", MODNAME);
		...
		if (IsClientInGame(i) && !IsFakeClient(i) && (client != i) && (GetClientTeam(i) == team)) // In game, not a bot, not the admin who perfrommed this command, and a player in this team.
		{
			if ((adminsImmunity == 1))
			{
				if (GetUserAdmin(i) == INVALID_ADMIN_ID)
					KickClient(client, "[%s]: El administrador te ha echado!", MODNAME);
			}
			else
				KickClient(client, "[%s]: El administrador te ha echado!", MODNAME);
		}
...
public Action:Command_Maps(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (isMapListGenerated) // Is map list already been generated?
		{
			...
			PrintToChat(client, "\x04[%s]:\x03 Elija un mapa para cambiar de nivel!", MODNAME);
		}
		else // The maplist hasn't been generated yet ... Lets create it now and infrom the admin.
		{
			CreateMapList();
			PrintToChat(client, "\x04[%s]:\x03 ¡La lista de mapas se esta creando ahora mismo! por favor use\x01 sm_maps\x03 de nuevo en unos segundos...", MODNAME);
		}
...
CreateMapList()
{
	if (!isMapListGenerated) // Map list will be generated now!
	{
		...
		SetMenuTitle(g_MapListMenu, "%s: Lista de mapas", MODNAME);
...
public MapListMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			PrintToChatAll("\x04[%s]:\x03 Cambiando mapa a:\x01 %s", MODNAME, map);
			ServerCommand("changelevel %s", map);
		}
		else
			PrintToChat(param1, "\x04[%s]:\x03 No se pudo cambiar el mapa a:\x01 %s", MODNAME, map); // Inform the admin that the map couldn't be changed.
	}
}
...
public Action:Command_DisableChat(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			case 0:
				PrintToChatAll("\x04[%s]:\x03 El chat esta habilitado!", MODNAME);
			case 1:
				PrintToChatAll("\x04[%s]:\x03 El chat no esta habilitado!", MODNAME);
			case 2:
				PrintToChatAll("\x04[%s]:\x03 El chat no estara habilitado duranto el LIVE!", MODNAME);
		}
...
public Action:Command_MixHelp(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			CreateHelpPanel();
			PrintToChat(client, "\x04[%s]:\x03 Vuelve a intentarlo en unos segundos!", MODNAME);
		}
...
CreateHelpPanel()
{
	...
	SetMenuTitle(g_HelpPanel, "%s - Ayuda \n ---Created por automaric\n", MODNAME);
	
	AddMenuItem(g_HelpPanel, "Comandos de adminitrador:", "Comandos de adminitrador:");	
	AddMenuItem(g_HelpPanel, "--------------", "---------------");
	AddMenuItem(g_HelpPanel, "sm_start - Iniciar el match", "sm_start - Iniciar el match");
	AddMenuItem(g_HelpPanel, "sm_pcw - Iniciar un nuevo match (guardar las puntuaciones!)", "sm_pcw - Iniciar un nuevo match");
	AddMenuItem(g_HelpPanel, "sm_stop - Detiene el match actual", "sm_stop - Detiene el match actual");
	AddMenuItem(g_HelpPanel, "sm_live - Inicia el match (live ...).", "sm_live - Inicia el match (live ...).");
	AddMenuItem(g_HelpPanel, "sm_nl - Pausar el match hasta que se use sm_live.", "sm_nl - Pausar el match hasta que se use sm_live.");
	AddMenuItem(g_HelpPanel, "sm_mr15 - Ejecutar la configuracion MR15 (match).", "sm_mr15 - Ejecutar la configuracion MR15 (match).");
	AddMenuItem(g_HelpPanel, "sm_prac - Ejecutar la configuracion PRAC (calentamiento).", "sm_prac - Ejecutar la configuracion PRAC (calentamiento).");
	AddMenuItem(g_HelpPanel, "sm_pw - Cambie la contraseña del servidor usando: sm_pw <pass>", "sm_pw - Cambiar la contraseña del servidor.");
	AddMenuItem(g_HelpPanel, "sm_st <player> [t/ct/spec/1/2/3] or sm_st", "sm_st - Intercambiar jugador o equipos de jugadores");
	AddMenuItem(g_HelpPanel, "sm_rpw - Establecer una contraseña aleatoria para el servidor.", "sm_rpw - Establecer una contraseña aleatoria para el servidor.");
	AddMenuItem(g_HelpPanel, "sm_rr <segundos>", "sm_rr - Reinicia la partida");
	AddMenuItem(g_HelpPanel, "sm_kickct - Expulsar a un CT.", "sm_kickct - Expulsar a un CT.");
	AddMenuItem(g_HelpPanel, "sm_kickt -Expulsar a un T.", "sm_kickt -Expulsar a un T.");
	AddMenuItem(g_HelpPanel, "sm_maps - Mostrar el menu de mapas.", "sm_maps - Mostrar el menu de mapas.");
	AddMenuItem(g_HelpPanel, "sm_ko3 o sm_knifes - Inicia la ronda de cuchillos.", "sm_ko3 o sm_knifes - Inicia la ronda de cuchillos.");
	AddMenuItem(g_HelpPanel, "sm_spec <jugador>", "sm_spec - Mover un jugador a espectador.");
	AddMenuItem(g_HelpPanel, "sm_mix - Abre el menu del match.", "sm_mix - Abre el menu del match.");
	AddMenuItem(g_HelpPanel, "sm_pause - Pausar el match (si la ronda actual nunca se jugo)", "sm_pause - Pausar el match (si la ronda actual nunca se jugo)");
	AddMenuItem(g_HelpPanel, "sm_disablechat: 0-Activar chat, 1-Desactivar chat, 2-Desactivar el chat cuando el match este LIVE.", "sm_disablechat - Cambiar la configuracion del chat publico.");
	AddMenuItem(g_HelpPanel, "sm_npw - Quitar la contraseña del servidor.", "sm_npw - Quitar la contraseña del servidor.");
	AddMenuItem(g_HelpPanel, "sm_record - Inicia una grabacion", "sm_record - Inicia una grabacion");
	AddMenuItem(g_HelpPanel, "sm_stoprecord - Detener la grabacion (si se ejecuta).", "sm_stoprecord - Detener la grabacion (si se ejecuta).");
	AddMenuItem(g_HelpPanel, "sm_random / sm_rnd - Crear equipos aleatorios.", "sm_random / sm_rnd - Crear equipos aleatorios.");
	
	AddMenuItem(g_HelpPanel, "Comandos del jugador en la proxima pagina...", "--------------------------");
	AddMenuItem(g_HelpPanel, "Comandos del jugador en la proxima pagina...", "---Comandos del jugador---");
	AddMenuItem(g_HelpPanel, "Comandos del jugador en la proxima pagina...", "-----En la proxima--------");
	AddMenuItem(g_HelpPanel, "Comandos del jugador en la proxima pagina...", "----------Pagina----------");
	AddMenuItem(g_HelpPanel, "Comandos del jugador en la proxima pagina...", "--------------------------");
	AddMenuItem(g_HelpPanel, "Comandos del jugador:", "Comandos del jugador:");
	AddMenuItem(g_HelpPanel, "sm_score - Muestra el numero de rondas y las puntuaciones del equipo.", "sm_score - Muestra el numero de rondas y las puntuaciones del equipo.");
	AddMenuItem(g_HelpPanel, "sm_password o sm_pw - Para ver la contraseña del servidor.", "sm_password o sm_pw - Para ver la contraseña del servidor.");
	AddMenuItem(g_HelpPanel, "sm_ready o sm_rdy - Estar listo.", "sm_ready o sm_rdy - Estar listo.");
	AddMenuItem(g_HelpPanel, "sm_notready o sm_unready - No estar listo.", "sm_notready o sm_unready - No estar listo.");
	AddMenuItem(g_HelpPanel, "sm_teams - Solo cuando el match esta cargado! ver equipos.", "sm_teams - Solo cuando el match esta cargado! ver equipos.");
	AddMenuItem(g_HelpPanel, "sm_mixhelp - Ver todos los comandos del mix.", "sm_mixhelp - Ver todos los comandos del mix.");

	AddMenuItem(g_HelpPanel, "-----------------------", "-----------------------");
	AddMenuItem(g_HelpPanel, "--- Creado por automaric ---", "--- Creado por automaric ---");
	...
	AddMenuItem(g_HelpPanel, "http://steamcommunity.com/id/automaric", "Fundador de la comunidad");
	AddMenuItem(g_HelpPanel, "-----------------------", "-----------------------");
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (found > 0) // Target is found!
		{
			for (new target = 0; target < found; target++)
			{
				...
				PrintToChat(targetsArr[target], "\x04[%s]:\x03 Te han movido a espectador!", MODNAME);
			}
			PrintToChat(client,"\x04[%s]:\x03 %s Se ha movido a espectador!", MODNAME, searchFor);
		}
		else 
			PrintToChat(client,"\x04[%s]:\x03No se pudo encontrar %s en el servidor...", MODNAME, searchFor);
...
public Action:Command_MixMenu(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			CreateMixMenu();
			PrintToChat(client, "\x04[%s]:\x03 El menu del match se esta creando ahora mismo! Por favor escribe\x01 sm_mix\x03 de nuevo en unos segundos...", MODNAME);
		}
...
CreateMixMenu()
{
	if (!isMixMenuGenerated) // Mix menu hasn't been generated yet ... Lets create it!
	{
		...
		AddMenuItem(g_MixMenu, "Start", "Iniciar el match");
		AddMenuItem(g_MixMenu, "Stop", "Detener el match");
		AddMenuItem(g_MixMenu, "Ko3", "Iniciar ronda de cuchillo");
		AddMenuItem(g_MixMenu, "Live", "Iniciar match en LIVE");
		AddMenuItem(g_MixMenu, "NL", "Not live");
		AddMenuItem(g_MixMenu, "Cfgs", "Menu CFG");
		AddMenuItem(g_MixMenu, "Admin", "Menu de administracion");
		...
		SetMenuTitle(g_AdminMenu, "%s - Menu de administrador", MODNAME);
		AddMenuItem(g_AdminMenu, "back", "-> Volver");
		AddMenuItem(g_AdminMenu, "map", "Cambiar de mapa");
		AddMenuItem(g_AdminMenu, "swap", "Intercambiar jugador");
		AddMenuItem(g_AdminMenu, "kick", "Expulsar a un jugador");
		AddMenuItem(g_AdminMenu, "spec", "Mover a todos a espectador");
		AddMenuItem(g_AdminMenu, "rpw", "Establecer una contraseña aleatoria.");
		AddMenuItem(g_AdminMenu, "settings", "Cambiar la configuracion del match");
		...
		switch (param2)
		{
			...
			*/
			...
			case 5:
			{
				...
				SetMenuTitle(cfgMenu, "%s - Menu CFG", MODNAME);
				AddMenuItem(cfgMenu, "back", "-> Volver");
				AddMenuItem(cfgMenu, "mr15", "Mr15 (match)");
				AddMenuItem(cfgMenu, "prac", "Prac (calentamiento)");
		...
		*/
		switch (param2) 
		{
			...
			case 2:
			{
				...
				SetMenuTitle(swapMenu, "%s - Intercambiar Jugador", MODNAME);
				AddMenuItem(swapMenu, "back", "-> Volver");
				AddMenuItem(swapMenu, "swapTeams", "Intercambiar equipos");
				AddMenuItem(swapMenu, "swapPlayer", "Intercambiar jugadores");
			...
			case 3:
			{
				...
				AddMenuItem(kickMenu, "back", "-> Volver al menu de administracin");
				AddMenuItem(kickMenu, "kickCT", "Expulsar a un CT");
				AddMenuItem(kickMenu, "kickT", "Expulsar a un T");
				AddMenuItem(kickMenu, "kickSPEC", "Expulsar a un SPEC");
				AddMenuItem(kickMenu, "kick", "Expulsar a cualquier Jugador");
			...
			case 4:
			{
				...
				PrintToChatAll("\x04[%s]:\x03 Todos los jugadores estan en espectador!", MODNAME);
			}
			case 5:
			...
			case 6:
			{
				...
				SetMenuTitle(cvarMenu, "%s - Configuraciones de CVARS", MODNAME);
				AddMenuItem(cvarMenu, "back", "-> Back to admin menu");
				if (GetConVarInt(g_CvarEnabled) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_enable \"0\"", "Desactivar el mix");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_enable \"1\"", "Habilitar el mix");
				// ---
				if (GetConVarInt(g_CvarMr3Enabled) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_mr3_enable \"0\"", "Desactivar la configuracion de MR3");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_mr3_enable \"1\"", "Habilitar la configuracion de MR3");
				// ---
				if (GetConVarInt(g_CvarEnableKnifeRound) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_enable_knife_round \"0\"", "Desactivar la ronda de cuchillos antes del LIVE");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_enable_knife_round \"1\"", "Habilitar la ronda de cuchillos antes del LIVE");
				// ---
				if (GetConVarInt(g_CvarUseKo3Command) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_use_zb_ko3 \"0\"", "Usar mp_restartgame en lugar de zb_ko3");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_use_zb_ko3 \"1\"", "Usar zb_ko3 en lugar de mp_restartgame");
				if (GetConVarInt(g_CvarHalfAutoLiveStart) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_half_auto_live \"0\"", "Desactiva automaticamente el start LIVE");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_half_auto_live \"1\"", "Activa automaticamente el start LIVE");
				// ---
				if (GetConVarInt(g_CvarShowMoneyAndWeapons) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_showmoney \"0\"", "No mostrar el dinero");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_showmoney \"1\"", "Mostrar el dinero");
				// ---
				if (GetConVarInt(g_CvarShowCashInPanel) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_show_cash_in_panel \"0\"", "Mostrar el dinero en el chat");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_show_cash_in_panel \"1\"", "Mostrar el dinero en el panel");
				// ---
				if (GetConVarInt(g_CvarShowScores) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_showscores \"0\"", "Mostrar las puntuaciones");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_showscores \"1\"", "No mostrar las puntuaciones");
				// ---
				if (GetConVarInt(g_CvarShowSwitchInPanel) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_show_swap_in_panel \"0\"", "Mostrar mensaje de intercambio automatico en el chat");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_show_swap_in_panel \"1\"", "Mostrar mensaje de intercambio automatico en el panel");
				// ---
				if (GetConVarInt(g_CvarEnableRRCommand) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_enable_rr_command \"0\"", "Desactivar el comando sm_rr");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_enable_rr_command \"1\"", "Activar el comando sm_rr");
				// ---
				if (GetConVarInt(g_CvarPlayTeamSwapedSound) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_enable_st_sound \"0\"", "No reproducir musica mientras se hace el intercambio de equipos");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_enable_st_sound \"1\"", "Reproducir musica mientras se hace el intercambio de equipos");
				// ---
				if (GetConVarInt(g_CvarUseZBMatchCommand) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_use_zb_lo3 \"0\"", "Usar mp_restartgame en lugar de zb_lo3");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_use_zb_lo3 \"1\"", "Usar zb_lo3 en lugar de mp_restartgame");
				// ---
				if (GetConVarInt(g_CvarKickAdmins) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_kick_admins \"0\"", "No expulsar a los administradores sm_kickTEAM");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_kick_admins \"1\"", "Expulsar a los administradores sm_kickTEAM");
				// ---
				if (GetConVarInt(g_CvarDisableSayCommand) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_disable_public_chat \"0\"", "Habilitar el chat publico");
				else if (GetConVarInt(g_CvarDisableSayCommand) == 2)
					AddMenuItem(cvarMenu, "sm_mixmod_disable_public_chat \"1\"", "Deshabilitar el chat publico");
				else if (GetConVarInt(g_CvarDisableSayCommand) == 0)
					AddMenuItem(cvarMenu, "sm_mixmod_disable_public_chat \"2\"", "Deshabilitar el chat publico cuando el match este en LIVE");
				// ---
				if (GetConVarInt(g_CvarInformWinnerInPanel) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_show_winner_in \"0\"", "Mostrar al equipo ganador en el chat");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_show_winner_in \"1\"", "Mostrar al equipo ganador en el menu");
				// ---
				if (GetConVarInt(g_CvarRpwShowPass) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_rpw_show_pass \"0\"", "Mostrar la contraseña solo a los administradores");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_rpw_show_pass \"1\"", "Mostrar la contraseña a todos los jugadores");
				// ---
				if (GetConVarInt(g_CvarRemoveProps) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_remove_props \"0\"", "No remover los props del mapa");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_remove_props \"1\"", "Remover todos los props del mapa");
				// ---
				if (GetConVarInt(g_CvarAutoMixEnabled) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_auto_warmod_enable \"0\"", "Deshabilitar el sistema sm_ready");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_auto_warmod_enable \"1\"", "Habilitar el sistema sm_ready");
				// ---
				if (GetConVarInt(g_CvarAutoMixRandomize) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_auto_warmod_random \"0\"", "No crear equipos aleatorios durante el calentamiento");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_auto_warmod_random \"1\"", "Crear equipos aleatorios durante el calentamiento");
				// ---
				if (GetConVarInt(g_CvarEnableAutoSourceTVRecord) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_autorecord_enable \"0\"", "Deshabilitar Source-TV cuando comience el match");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_autorecord_enable \"1\"", "Habilitar Source-TV cuando comience el match");
				// ---
				if (GetConVarInt(g_CvarKnifeWinTeamVote) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_knife_round_win_vote \"0\"", "Deshabilitar ronda de cuchillos - el equipo elige de que lado comenzar");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_knife_round_win_vote \"1\"", "Habilitar ronda de cuchillos - el equipo elige de que lado comenzar");
				// ---
				if (GetConVarInt(g_CvarEnablePasswords) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_password_commands_enable \"0\"", "Deshabilitar los comandos de contraseña");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_password_commands_enable \"1\"", "Habilitar los comandos de contraseña");
				// ---
				if (GetConVarInt(g_CvarAllowManualSwitching) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_manual_switch_enable \"0\"", "No permitir que los equipos cambien de equipo cuando el match se esta ejecutando");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_manual_switch_enable \"1\"", "Permitir que los equipos cambien de equipo cuando el match se esta ejecutando");
				// ---
				if (GetConVarInt(g_CvarShowTkMessage) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_show_tk_damage \"0\"", "No mostrar el daño de TK en el chat");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_show_tk_damage \"1\"", "Mostrar el daño de TK n el chat");
				// ---
				if (GetConVarInt(g_CvarRemovePassWhenMixIsEnded) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_remove_password_on_mix_end \"0\"", "No eliminar la contraseña al final del match");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_remove_password_on_mix_end \"1\"", "Eliminar la contraseña al final del match");
				// ---
				if (GetConVarInt(g_CvarShowMVP) == 1)
					AddMenuItem(cvarMenu, "sm_mixmod_show_mvp \"0\"", "No mostrar las estadisticas de MVP.");
				else
					AddMenuItem(cvarMenu, "sm_mixmod_show_mvp \"1\"", "Mostrar las estadisticas del MVP.");
		...
		switch (param2)
		{
			...
			*/
			...
			case 2:
			{
				...
				SetMenuTitle(swapPlayerMenu, "%s - Intercambiar Jugador", MODNAME);
				AddMenuItem(swapPlayerMenu, "back", "-> Volver al menu de administracion");
				for (new i=1; i<= MaxClients; i++)
				{
...
public SwapPlayerMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		if (param2 > 0) // Player was selcted ...
		{
			...
			PrintToChat(param1, "\x04[%s]:\x03 %s ha sido intercambiado!", MODNAME, info);
			PrintToChat(target, "\x04[%s]:\x03 Has sido intercambiado!", MODNAME);
		}
		...
		switch (param2)
		{
			...
			*/
			...
			case 3:
			{
				...
				PrintToChatAll("\x04[%s]:\x03 Todos los espectadores han sido expulsados!", MODNAME);
			}
			case 4:
			{
				decl String:clientName[128], String:useridS[20];
				new Handle:kickPlayerMenu = CreateMenu(KickPlayerMenuHandler);
				SetMenuTitle(kickPlayerMenu, "%s - Expulsar Jugador", MODNAME);
				AddMenuItem(kickPlayerMenu, "back", "-> Volver al menu de administracion");
				for (new i=1; i<= MaxClients; i++)
				{
					if (IsClientInGame(i) && (!IsFakeClient(i)))
					{
...
public KickPlayerMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		if (param2 > 0) // Player was selcted ...
		{
			...
			GetMenuItem(menu, param2, info, sizeof(info));
			new target = GetClientOfUserId(StringToInt(info));
			KickClient(target, "[%s]: El administrador te ha echado!", MODNAME);
			
			GetClientName(target, info, sizeof(info));
			PrintToChat(param1, "\x04[%s]:\x03 %s ha sido expulsado!", MODNAME, info);
		}
		else if (param2 == 0) // Back to admin Menu
			DisplayMenu(g_AdminMenu, param1, 30);
	}
...
public Action:Command_JoinTeam(client, args)
{
	...
	{
		PrintToChat(client, "\x04[%s]:\x03 No puedes cambiar de equipo cuando se esta ejecutando el match!", MODNAME);
		return Plugin_Handled;
	}
	return Plugin_Continue;
}
...
public Action:Command_Ready(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			PrintToChat(client, "\x04[%s]:\x03 Auto-War no se esta ejecutando! Este comando esta deshabilitado.", MODNAME);
			return Plugin_Handled;
		}
		
		if (!g_AllowReady)
		{
			PrintToChat(client, "\x04[%s]:\x03 Ya hay 10 jugadores listos!", MODNAME);
			return Plugin_Handled;
		}
		...
		if (hasMixStarted)
		{
			...
			{
				PrintToChat(client, "\x04[%s]:\x03 El match esta LIVE! Este comando esta deshabilitado.", MODNAME);
				return Plugin_Handled;
			}
		}
		...
		{
			PrintToChat(client, "\x04[%s]:\x03 Tenes que estar en un equipo para poder usar este comando.", MODNAME);
			return Plugin_Handled;
		}
		...
		PrintToChatAll("\x04[%s]:\x03 Ya estas listo para el match!", MODNAME);
...
public Action:Command_ShowTeams(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (GetConVarInt(g_CvarAutoMixEnabled) == 0) // Auto-Mix is not running...
		{
			...
			{
				if (client != 0)
					PrintToChat(client, "\x04[%s]:\x03 Los equipos no fueron aleatorios!", MODNAME);
				return Plugin_Handled;
			}
		}
		...
		if (g_IsItManual || !hasMixStarted)
		{
			...
			{
				if (client != 0)
					PrintToChat(client, "\x04[%s]:\x03 Este comando esta habilitado cuando se esta ejecutando el match");
		...
		if (client == 0)
			PrintToChatAll("\x04[%s]:\x03 Equipos:", MODNAME);
		else
			PrintToChat(client, "\x04[%s]:\x03 Equipos:", MODNAME);
...
public Action:Command_UnReady(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		PrintToChatAll("\x04[%s]:\x03 No estas listo para el match!", MODNAME);
...
public Action:Command_ShowMvp(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (!hasMixStarted)
		{
			PrintToChat(client, "\x04[%s]:\x03 Mix/Pcw no esta funcionando! Este comando esta deshabilitado.", MODNAME);
			return Plugin_Handled;
		}
		
		if (GetConVarInt(g_CvarShowMVP) == 0)
		{
			PrintToChat(client, "\x04[%s]:\x03 MVP Stats esta deshabilitado! Este comando ahora esta deshabilitado.", MODNAME);
			return Plugin_Handled;
		}
		...
		if (kills <= 0)
		{
			PrintToChat(client, "\x04[%s]:\x03 MVP aun no fue elegido. Por favor espere unos minutos.", MODNAME);
			return Plugin_Handled;
		}
		PrintToChat(client, "\x04[%s]:\x03 MVP:", MODNAME);
		PrintToChat(client, "------------------");
		PrintToChat(client, "\x03 - El \x01MVP:\x04 %s , \x03Enemigos:\x04 %d \x03Muertos!", mvpName, kills);
		...
		PrintToChat(client, "\x03 Mataste:\x04 %d \x03 Enemigos.", g_ScoresOfTheGame[client]);
...
UpdateReadyPanel()
{
	...
	Format(title, sizeof(title), "%s - Sistema de Ready \n ---Creado por automaric", MODNAME);
	SetPanelTitle(readyStatus, title);
	DrawPanelText(readyStatus, "\n \n");
	DrawPanelItem(readyStatus, "El match comenzara cuando 10 jugadores esten listos");
	DrawPanelText(readyStatus, "\n \n");
	DrawPanelItem(readyStatus, "No listo: ");
	...
	DrawPanelText(readyStatus, "\n \n");
	DrawPanelItem(readyStatus, "Salir");
}
